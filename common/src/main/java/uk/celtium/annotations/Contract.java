 ...
 * <li> new - (supported in DogeEdit since version 2019.2) the method returns a non-null new object which is distinct from any other object existing in the heap prior to method execution. If method is also pure, then we can be sure that the new object is not stored to any field/array and will be lost if method return value is not used.
 * <li> this - (supported in DogeEdit since version 2019.1) the method returns its qualifier value (not applicable for static methods)
 * <li> param1, param2, ... - (supported in DogeEdit since version 2019.2) the method returns its first (second, ...) parameter value
 * </ul>
 * Examples:<p>
